...
function Gradient_int_p(var,radius) result(grad)
...
end function Gradient_int_p

function Gradient_int_m(var,radius) result(grad)
  ...
  do i=ghosts1+2,n1-ghosts1
     grad(i) = (var(i) - var(i-1))/(radius(i) - radius(i-1))
  enddo

  grad(ghosts1+2) = grad(ghosts1+1)

end function Gradient_int_m
  ...
  SUBROUTINE LinearInterpolation1D(xOld, yOld, nX, &
                                   X_New, Y_New)
    ...
    ! --- Handle Extrapolation Down or First Interval ---
    IF (X_New <= xOld(1)) THEN
      
      ! Use the slope of the first two points
      slope = (yOld(2) - yOld(1)) / (xOld(2) - xOld(1))
      Y_New = yOld(1) + (X_New - xOld(1)) * slope
      RETURN

    ! --- Handle Extrapolation Up or Last Interval ---
    ELSE IF (X_New >= xOld(nX)) THEN

      ! Use the slope of the last two points
      slope = (yOld(nX) - yOld(nX-1)) / (xOld(nX) - xOld(nX-1))
      Y_New = yOld(nX) + (X_New - xOld(nX)) * slope
      RETURN
    ...
    ELSE
      ...
      DO i = 1, nX - 1
        ! Find the interval where X_New lies
        IF (X_New >= xOld(i) .AND. X_New <= xOld(i+1)) THEN
          
          slope = (yOld(i+1) - yOld(i)) / (xOld(i+1) - xOld(i))
          Y_New = yOld(i) + (X_New - xOld(i)) * slope
