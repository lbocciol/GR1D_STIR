...
subroutine M1_closure
  ...
  real*8 :: alp2(nM1),invalp(nM1),invalp2(nM1),onealp(nM1)
  real*8 :: X2(nM1),invX(nM1),invX2(nM1),oneX(nM1)
  real*8 :: W2(nM1),v2(nM1),oneW(nM1),onev(nM1)
  real*8 :: udown(2,nM1)
  real*8 :: littlehupdown(2,2,nM1),Tupmunu(2,2)
        ...
        do i=1,number_species_to_evolve
           do j=1,number_groups
              ...
              if (h.eq.1) then !minus state
                 ...
                 oneM1flux = q_M1m(j,k,i,2,1)/q_M1m(j,k,i,1,1)
                 oneM1eddy_guess = q_M1(j,k,i,3)
              else if (h.eq.2) then !middle state
                 oneM1en = q_M1(j,k,i,1)/q_M1(j,k,i,1)
                 oneM1flux = q_M1(j,k,i,2)/q_M1(j,k,i,1)
                 oneM1eddy_guess = q_M1(j,k,i,3)
              else if (h.eq.3) then !plus state
                 oneM1en = q_M1p(j,k,i,1,1)/q_M1p(j,k,i,1,1)
                 oneM1flux = q_M1p(j,k,i,2,1)/q_M1p(j,k,i,1,1)
                 oneM1eddy_guess = q_M1(j,k,i,3)
              endif
              ...
              count = 0
              Tupmunu(1,1) = oneM1en*invalp2(k)
              Tupmunu(1,2) = oneM1flux*invX2(k)*invalp(k)
              Tupmunu(2,1) = Tupmunu(1,2)

              do while (err>tol)
                 count = count + 1
                 Tupmunu(2,2) = oneM1eddy_guess*oneM1en*invX2(k)**2
                 ...
                 do ii=1,2
                    do jj=1,2
                       localJ = localJ + udown(ii,k)*udown(jj,k)*Tupmunu(ii,jj)
                       Hup(1) = Hup(1) - udown(ii,k)*littlehupdown(1,jj,k)*Tupmunu(ii,jj)  
                       Hup(2) = Hup(2) - udown(ii,k)*littlehupdown(2,jj,k)*Tupmunu(ii,jj)  
                    enddo
                 enddo
                 ...
                 do ii=1,2
                    do jj=1,2
                       if (ii.eq.1) then
                          H2 = H2 - alp2(k)*littlehupdown(ii,jj,k)*Hup(ii)*Hup(jj)
                       else
                          H2 = H2 + X2(k)*littlehupdown(ii,jj,k)*Hup(ii)*Hup(jj)
                       endif
                    enddo
                 enddo

                 ff2 = ((onealp(k)*onev(k)*oneW(k)*Hup(1))**2 + (oneX(k)*oneW(k)*Hup(2))**2 - &
                      2.0d0*onealp(k)*W2(k)*oneX(k)*onev(k)*Hup(2)*Hup(1))/localJ**2*0.999999999999d0

                 if (ff2.ne.ff2) then
                    if (localJ**2.eq.0.0d0) then
                       ff2 = 0.0d0
                    else
                       write(*,*) oneM1en,oneM1flux,nt,j,k,i
                       stop "ff2 is NaNing"
                    endif
                 ...
                 Kthick(2,2) = localJ*onethird*littlehupdown(2,2,k)*invX2(k)

                 Kup(2,2) = (3.0d0*chi-1.0d0)/2.0d0*Kthin(2,2) + 3.0d0*(1.0d0-chi)/2.0d0*Kthick(2,2)

                 Tupmunu(2,2) = ((oneW(k)*onev(k)*oneX(k))**2*localJ +  &
                      2.0d0*oneW(k)*X2(k)*onev(k)*oneX(k)*Hup(2) + X2(k)*X2(k)*Kup(2,2))*invX2(k)**2
                 
                 oldprrguess = oneM1eddy_guess
                 oneM1eddy_guess = Tupmunu(2,2)*X2(k)*X2(k)/oneM1en
                 ...
                 if (count.gt.1000) then
                    if (err.lt.1.0d-5) then
                       write(*,*) "accepting lower error", err,j,k,i,h,onev(k),oneM1eddy_guess,Hup(2)
                       err = 1.0d-9
                    else
                       if (chi.gt.0.9d0) then
                          write(*,*) "forcing p_rr to X^2, this should only happen near the shock at bounce",k,chi
                          write(*,*) j,k,i,h,oneM1en,oneM1flux,oneM1flux/oneX(k),q_M1(j,k,i,3),X2(k)
                          oneM1eddy_guess = X2(k)
                          err = 1.0d-9
                       else
                          if (count.gt.1000) then
                             write(*,*) "Error at",count,":", err,j,k,i,h,chi
              ...
              if (h.eq.1) then !minus state
                 q_M1m(j,k,i,3,1) = oneM1eddy_guess !P_{rr}/E
                 q_M1_extram(j,k,i,1,1) = chi
              else if (h.eq.2) then !middle state
                 if (Hup(2).eq.0.0d0) then
                    Lthin = 0.0d0 !Cardall 108
                 else
                    Lthin = Hup(2)*invX2(k) !Cardall 108
                 endif
                 Lthick = 0.6d0*Hup(2)*littlehupdown(2,2,k)*invX2(k) !Cardall 108
                 Luprrr = (3.0d0*chi-1.0d0)*0.5d0*Lthin + 3.0d0*(1.0d0-chi)*0.5d0*Lthick
                 Ldownfupfr = 3.0d0*(1.0d0-chi)*0.5d0*0.2d0*Hup(2) !Cardall 108

                 Wuprrr = Luprrr + 3.0d0*oneW(k)*onev(k)*invX(k)*Kup(2,2) +  &
                      3.0d0*W2(k)*v2(k)*invX2(k)*Hup(2) + W2(k)*oneW(k)*v2(k)*onev(k)*invX2(k)*invX(k)*localJ !cardall B18
                 Wdownfupfr = oneW(k)*onev(k)*invX(k)*3.0d0*(1.0d0-chi)*0.5d0*localJ*onethird + & 
                      Ldownfupfr !cardall B18

                 q_M1(j,k,i,3) = oneM1eddy_guess !P_{rr}/E
                 q_M1_extra(j,k,i,1) = 3.0d0*(1.0d0-chi)*0.5d0* &
                      localJ*onethird/oneM1en !P_{phi}^{phi}/E,P_{\theta}^{\theta}/E,
                                              !note oneM1en is 1
                 q_M1_extra(j,k,i,4) = chi
                 q_M1_extra(j,k,i,2) = Wuprrr*q_M1(j,k,i,1)
                 q_M1_extra(j,k,i,3) = Wdownfupfr*q_M1(j,k,i,1)
              else if (h.eq.3) then !plus state
                 q_M1p(j,k,i,3,1) = oneM1eddy_guess !P_{rr}/E
                 q_M1_extrap(j,k,i,1,1) = chi
              endif
           enddo
        enddo
     enddo
